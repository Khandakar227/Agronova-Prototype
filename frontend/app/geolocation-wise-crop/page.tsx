...
const suitabilityMapping = {
  "Not Suitable": { en: "Not Suitable", color: "rgba(239, 68, 68, 1)", textColor: "black" },
  "Moderately Suitable": { en: "Moderately Suitable", color: "rgba(253, 224, 71, 1)", textColor: "black" },
  "Suitable": { en: "Suitable", color: "rgb(0 216 81)", textColor: "black" },
  "Very Suitable": { en: "Very Suitable", color: "rgba(21, 128, 61, 1)", textColor: "white" },
};
    ...
    fetch(weatherUrl)
      ...
      .then((data) => {
        setWeather({
          ...
          weatherType: weatherTypeEn[data.weather[0].description as keyof typeof weatherTypeEn] || "Unknown",
        });
      })
      .catch((err) => console.error(err));
  }, [selectedLocation]);
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-less-dark relative">
      ...
      <header className="text-center py-6 bg-green-700 text-white">
        ...
        <p>Providing accurate agricultural advice and weather updates based on your location.</p>
      </header>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {selectedLocation && (
          <div
            className={`fixed top-20 right-0 ${suitabilityVisible
          ...
          >
            {/* Toggle Button */}
            <button
              className={`${suitabilityVisible ? "absolute top-6 right-6 dark:bg-less-dark" : ""
                  ...
                  {suitability &&
                    Object.keys(suitability).map((crop, i) => (
                      <li key={i} className="flex justify-between">
                        <span>
                          {crops_names_en[crop as keyof typeof crops_names_en]}
                        </span>
                        <span
                          style={{
                            backgroundColor:
        ...
        {weather && (
          <div
            className={`fixed top-20 left-0  ${weatherVisible ? "w-80 p-6 bg-white dark:bg-less-dark dark:text-gray-200 shadow-lg rounded-r-lg" : "w-16 h-16 shadow-md shadow-green-700 bg-green-500 dark:bg-lime-500 rounded-r-full"
              }`}
            style={{
          ...
          >
            {/* Toggle Button */}
            <button
              className={`${weatherVisible ? "absolute top-6 left-3 dark:bg-less-dark" : ""
                } rounded-full p-1 bg-white shadow border`}
